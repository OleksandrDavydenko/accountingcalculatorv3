...
export const calculations = (incomeData) => {
  ...
  for (let i = 0; i < calculatedData.length; i++) {
    ...
    calculatedData[i].numberOfMonth = monthDiff(
      new Date(calculatedData[i].aremeentDate),
      new Date(calculatedData[i].aremeentDateEnd)
    );

    // вартість договору

    element.agreementSum = (
      element.numberOfMonth * element.monthPayment
    ).toFixed(2);
    ...
    function discontValue(params) {
      ...
      for (let index = 0; index < calculatedData[i].numberOfMonth; index++) {
        calculatedData[i].currentValue = (
          (previosElementMonthPayment * 100) /
          (100 + Number(element.discountRate) / 12)
        ).toFixed(2);
    ...
    element.monthAmort = (element.currentValue / element.numberOfMonth).toFixed(
      2
    );

    //Фінансові витрати

    element.financeCosts = (
      element.agreementSum - element.currentValue
    ).toFixed(2);
  }
